...
const LAYER_H = 30
/** The preset lane is dragged too, and carries a name, so it matches the layer lanes. */
const PRESET_H = 30
 ...
 *
 * Lanes, top to bottom in the order things are DRAWN: captions (the one that matters), the preset
 * lane that says which stretches of them use a different look, the two media layers (images and
 * clips over the video — Layer 2 on top of Layer 1), then the video and audio underneath. Layer items are real clips: select, drag, trim by an edge, split with the
...
export function Timeline({
  ...
  const presetSegments = usePresetSegments()
    ...
    <div className="flex min-h-0 shrink-0 flex-col border-t border-border/60 bg-card">
      ...
      <div className="flex min-h-0 flex-1">
        ...
        >
          ...
          <TrackHeader name="Preset" icon={Palette} height={PRESET_H} />
        ...
        >
          <div className="relative" style={{ width: durationMs * effectiveZoom }}>
            ...
            />
            <PresetLane
              segments={presetSegments.segments}
              ...
              height={PRESET_H}
              playheadMs={timeMs}
              onChange={presetSegments.setSegment}
              onRemove={presetSegments.removeSegment}
              onSeek={handleSeek}
            />
